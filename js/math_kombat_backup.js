...
function drawFighter(f, ts, W, H, floorY) {
    ...
    // Hit burst effect (on top of character)
    if (f.state === 'hit') {
        ctx.globalAlpha = 0.8;
        const burstR = 18 + Math.sin(ts * 0.03) * 5;
        const grd = ctx.createRadialGradient(5, -55, 2, 5, -55, burstR);
        ...
        ctx.lineWidth = 2;
        for (let i = 0; i < 8; i++) {
            const a = (i / 8) * Math.PI * 2 + ts * 0.01;
            ctx.beginPath();
            ctx.moveTo(5 + Math.cos(a) * 8, -55 + Math.sin(a) * 8);
            ctx.lineTo(5 + Math.cos(a) * (burstR + 5), -55 + Math.sin(a) * (burstR + 5));
            ctx.stroke();
        }
        ctx.globalAlpha = 1;
    }
...
function _drawLegs(ctx, f, pantsC, shoeC, spread, shoeW) {
    ...
    if (f.state === 'kick') {
        // Standing leg
        ctx.fillStyle = pantsC; _limb(ctx, -sp, -5, 9, 32);
        ctx.fillStyle = shoeC; _limb(ctx, -sp, 27, sw, 8);
        // Kick leg extended
        ctx.fillStyle = pantsC;
        ctx.save(); ctx.translate(sp, -10); ctx.rotate(-Math.PI / 4.5);
        ctx.fillRect(-4, -42, 9, 42); ctx.restore();
        ctx.fillStyle = shoeC;
        ctx.save(); ctx.translate(sp + 24, -48); ctx.fillRect(-5, -3, sw + 2, 7); ctx.restore();
    } else {
        // Both legs - fighting stance
        ctx.fillStyle = pantsC;
        _limb(ctx, -sp, -5, 9, 34); _limb(ctx, sp, -5, 9, 34);
        ctx.fillStyle = shoeC;
        _limb(ctx, -sp, 29, sw, 8); _limb(ctx, sp, 29, sw, 8);
    }
}
function _drawArms(ctx, f, armColor, fistColor, torsoTop) {
    ...
    if (f.state === 'punch') {
        // Punching arm
        ctx.save(); ctx.translate(18, ty + 8);
        ctx.fillRect(0, -4, 55, 9);
        ctx.fillStyle = fistColor;
        ctx.fillRect(53, -6, 13, 13);
        ctx.restore();
        // Guard arm
        _limb(ctx, -22, ty, 8, 26);
        ctx.fillStyle = fistColor; _limb(ctx, -22, ty + 24, 10, 10);
    } else if (f.state === 'special') {
        // Both arms thrust forward
        ctx.save(); ctx.translate(10, ty + 12);
        ctx.fillRect(0, -5, 45, 9);
        ctx.fillRect(0, 6, 45, 9);
        ctx.restore();
    } else if (f.state === 'drink') {
        // Drinking potion - one arm holds bottle to mouth, other at side
        // Arm at side
        _limb(ctx, -22, ty, 8, 26);
        ctx.fillStyle = fistColor; _limb(ctx, -22, ty + 24, 10, 10);
        // Arm bringing potion to face
        ctx.fillStyle = armColor;
        ctx.save(); ctx.translate(14, ty + 4);
        ctx.rotate(-1.2); // arm bent up
        ctx.fillRect(0, -4, 8, 28);
        // Potion bottle
        ctx.fillStyle = '#22c55e'; // green potion
        _roundRect(ctx, -4, -18, 14, 16, 4); ctx.fill();
        // Bottle neck
        ctx.fillStyle = '#16a34a';
        ctx.fillRect(1, -24, 6, 8);
        // Liquid glow
        ...
        ctx.restore();
        // HP recovery particles floating up
        ...
        // Guard stance
        _limb(ctx, -22, ty, 8, 26);
        _limb(ctx, 20, ty, 8, 22);
        ctx.fillStyle = fistColor;
        _limb(ctx, -22, ty + 24, 10, 10);
        _limb(ctx, 20, ty + 20, 10, 10);
    }
}
...
CHAR_DRAWERS['ryuken'] = (ctx, f, ch, ts) => {
    ...
    // Eyes (intense, angry)
    _eyes(ctx, -97, true);
    // Determined mouth (gritted teeth)
    ctx.fillStyle = '#a06040'; ctx.fillRect(-4, -84, 8, 2);
    ctx.fillStyle = '#fff'; ctx.fillRect(-3, -84, 6, 1);
    // SPECIAL: Hadouken
    if (f.state === 'special') {
...
CHAR_DRAWERS['scorpius'] = (ctx, f, ch, ts) => {
    ...
    // WHITE GLOWING EYES (menacing)
    ctx.fillStyle = '#fff';
    ctx.shadowColor = '#fbbf24'; ctx.shadowBlur = 10;
    ctx.fillRect(-9, -96, 7, 5);
    ctx.fillRect(3, -96, 7, 5);
    ctx.shadowBlur = 0;
...
CHAR_DRAWERS['subfrost'] = (ctx, f, ch, ts) => {
    ...
    // ICE-BLUE GLOWING EYES (brighter)
    ctx.fillStyle = '#7dd3fc';
...
CHAR_DRAWERS['gokhan'] = (ctx, f, ch, ts) => {
    ...
    ctx.shadowBlur = 0;
    // Eyes (teal in SSJ form)
    ctx.fillStyle = '#fff';
    ctx.fillRect(-10, -97, 8, 5); ctx.fillRect(3, -97, 8, 5);
    ctx.fillStyle = '#14b8a6'; // teal
    _circle(ctx, -6, -95, 2.5); _circle(ctx, 7, -95, 2.5);
    // Angry brows (SSJ intensity)
    ctx.fillStyle = '#000';
    ctx.save(); ctx.translate(-6, -100); ctx.rotate(-0.25); ctx.fillRect(-5, 0, 10, 2); ctx.restore();
    ctx.save(); ctx.translate(7, -100); ctx.rotate(0.25); ctx.fillRect(-5, 0, 10, 2); ctx.restore();
    // Determined grin
...
CHAR_DRAWERS['veggan'] = (ctx, f, ch, ts) => {
    ...
    // Eyes (intense, deep-set, on blue skin)
    ctx.fillStyle = '#fff';
    ctx.fillRect(-9, -97, 7, 4); ctx.fillRect(3, -97, 7, 4);
    ctx.fillStyle = '#111';
    _circle(ctx, -6, -95, 2); _circle(ctx, 6, -95, 2);
    // Very angry brows
    ctx.fillStyle = '#1a1a1a';
...
CHAR_DRAWERS['milena'] = (ctx, f, ch, ts) => {
    ...
    // Sharp teeth visible through mask!
    ctx.fillStyle = '#fff';
    for (let i = -4; i <= 4; i += 2) {
        ctx.beginPath();
        ctx.moveTo(i - 1, -80); ctx.lineTo(i, -77); ctx.lineTo(i + 1, -80);
        ctx.closePath(); ctx.fill();
    }
    // Fierce yellow eyes (Tarkatan)
    ctx.fillStyle = '#fef08a';
    ctx.shadowColor = '#fbbf24'; ctx.shadowBlur = 5;
    ctx.fillRect(-9, -96, 7, 4);
    ctx.fillRect(3, -96, 7, 4);
    ctx.fillStyle = '#854d0e';
    _circle(ctx, -5, -94, 1.5); _circle(ctx, 6, -94, 1.5);
    ctx.shadowBlur = 0;
    // SPECIAL: Sai throw
    if (f.state === 'special') {
        ctx.fillStyle = '#d4d4d4';
...
function updateHUD() {
    ...
    hp1.className = `hp-bar-inner ${game.p1.hp > 50 ? 'hp-green' : game.p1.hp > 20 ? 'hp-yellow' : 'hp-red'}`;
    hp2.className = `hp-bar-inner ${game.p2.hp > 50 ? 'hp-green' : game.p2.hp > 20 ? 'hp-yellow' : 'hp-red'}`;
    ...
    const s1 = document.getElementById('atk-special-p1');
    const u1 = document.getElementById('atk-super-p1');
    const s2 = document.getElementById('atk-special-p2');
    const u2 = document.getElementById('atk-super-p2');
    s1.classList.toggle('locked', game.p1.streak < 3);
    u1.classList.toggle('locked', game.p1.streak < 5);
    s2.classList.toggle('locked', game.p2.streak < 3);
    u2.classList.toggle('locked', game.p2.streak < 5);
}
...
function scheduleCpuBuzzer() {
    ...
    cpuTimerId = setTimeout(() => {
        ...
        if (game.questionActive && !game.turn) {
            handleBuzzer('p2');
            // CPU answers after a short think delay
...
function handleBuzzer(pid) {
    ...
    document.getElementById('question-text').textContent += ` ← ${CHARACTERS[game[pid].charIdx].name} responde!`;
...
function handleAnswer(pid, val, btnEl) {
    ...
    if (correct) {
        ...
        setTimeout(() => {
            ...
            if (game[pid].streak < 3) {
                executeAttack(pid, 'normal');
            } else {
                document.getElementById(`attack-menu-${pid}`).classList.remove('hidden');
                // CPU auto-picks attack
        ...
        setTimeout(() => {
            ...
            if (game[other].streak < 3) {
                ...
                document.getElementById(`attack-menu-${other}`).classList.remove('hidden');
                // CPU auto-picks attack when opponent errs
                if (game.mode === 'hvc' && other === 'p2') {
                    setTimeout(() => {
                        const atkType = cpuChooseAttack('p2');
                        const atkBtn = document.querySelector(`#attack-menu-p2 .atk-${atkType}`);
...
document.querySelectorAll('.atk-btn').forEach(btn => {
    btn.addEventListener('pointerdown', (e) => {
        ...
        executeAttack(pid, type);
    });
});

function executeAttack(attackerId, type) {
    if (type === 'special' && game[attackerId].streak < 3) return;
    if (type === 'super' && game[attackerId].streak < 5) return;
    ...
    if (type === 'normal') {
        // ============ CLOSE-RANGE PUNCH ============
        // Walk up right next to defender for real contact
        const contactOffset = 0.06; // How close they get
        ...
        // Phase 1: Walk towards opponent (smooth lerp)
        atkFighter.targetX = contactX;

        // Phase 2: After arriving, punch
        setTimeout(() => {
        ...
        // Phase 4: Walk back to original position
        setTimeout(() => {
        ...
        // Move forward a bit, then kick/projectile
...
function piReveal() {
    ...
    setTimeout(() => {
        ...
        // If CPU won, CPU auto-answers
        if (game.mode === 'hvc' && winner === 'p2') {
            setTimeout(() => cpuAnswer(), 800 + Math.random() * 600);
        }
    }, 2000);
}
